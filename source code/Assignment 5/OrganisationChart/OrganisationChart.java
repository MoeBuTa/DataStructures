...
public class OrganisationChart {
	...
	PrintStream out;{
		try {
			out = new PrintStream("OrganisationText.txt");		
		} catch (FileNotFoundException e) {e.printStackTrace();}
	}												//create a printStream to save the organisation tree
    ...
    public void setupGUI(){
        ...
        UI.addButton("print tree", ()->{printTree(organisation, "");});
        UI.addButton("Save tree", ()->{saveTree(organisation,out);});
    ...
    private Employee findEmployee(double x, double y, Employee empl){
        if (empl.on(x, y)) {   // base case: (x,y) is on root of subtree       	
        	...
        	for(Employee e : empl.getTeam()) {       		
        		Employee ans = findEmployee(x,y,e);
        		if(ans != null) {        			
        			return ans;
        		}
        	}       	
    ...
    private void moveEmployee(Employee empl, Employee target) {
        ...
        /*# YOUR CODE HERE */
            if(inSubtree(target, empl)) {return;} //don't move the employee if the target is in the subtree of the employee
            if(!target.isVacant()) {           	
    ...
    public void retirePerson(Employee empl){
        /*# YOUR CODE HERE */
    	if(empl.isManager() || empl.getManager() == null) {
    		empl.makeVacant();
    	}
    	else {
    		empl.removeFromTeam(empl);
    	}
    }
    ...
    */
    private boolean inSubtree(Employee person, Employee subTree) {
        ...
        if(person.getManager()!=null) {
        	boolean in = inSubtree(person.getManager(), subTree);	
        	return in;
        }
        return false;
    }
    ...
    public void saveTree(Employee empl, PrintStream out) {
    	if(empl.getManager()!=null) {		
    		out.println(empl.toStringFull()+" "+empl.getManager().getInitials());
    	}
    	else {out.println(empl.toStringFull()+" null");}
		if(organisation.getTeam()!=null) {
          for(Employee e : empl.getTeam()) {
        	  saveTree(e,out);
          }
		}
    ...
    public void loadTree() {
    	UI.clearGraphics();
    	Set<Employee> tree = new HashSet<Employee>();
    	
    	Scanner sc;
		try {
			sc = new Scanner(new File("OrganisationText.txt"));
		...
		while(sc.hasNext()) {
			...
			Employee empl = new Employee(initial, role ,offset);    		
			tree.add(empl);
			for(Employee e : tree) {
				if(e.getInitials().equals(manager)) {
					e.addToTeam(empl);
				}
			}
		}
