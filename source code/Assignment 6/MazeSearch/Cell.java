 ...
 * It is a node in a graph of Cells - each Cell has a set of neighbours.
 * The useful methods are:
 *  - visit(), unvisit(), and isVisited()
 *    (to record if a Cell has been visted during a search)
...
public class Cell implements Iterable<Cell>{
    ...
    /**
     * Make a Cell located at the given row and column
     */
    public Cell(int row, int col) {
        this.row = row;
        this.col = col;
        neighbours = new HashSet<Cell>(4); 
    }
    ...
    /**
     * Allows a foreach loop to step through the neighbours of a Cell:
     *  for (Cell neigh : cell) {..do something to each neighbour...}
     */
