...
public class Permutations {
    ...
    public void setupGUI(){
        ...
        UI.addButton("Iterate Permute", this::findPermutationsIterative);
        UI.addButton("Clear display", this::clear);      
        this.Button = UI.addButton("Pause", this::pausePermutation);
    ...
    public void pausePermutation() {    	
    	pause = !pause;  
    	if(pause == true) {		//Change the background color of the pause button to represent the button is hit or not
    ...
    //create a find permutations method use an iterative way
    public void findPermutationsIterative() {
    	Set<String> select = new HashSet<String>(selectedList);
    	Iterator<String> it = select.iterator();
    	getPermutation(select, new ArrayList<String>());
    	 UI.printMessage(String.format("FINISHED: %d  items: %,d permutations",
                 select.size(), permutationCount));
    }
    
    //find all permutations in a iterative way
    public void getPermutation(Set<String> select, List<String> permute) {
    	Iterator<String> it = select.iterator();
    	System.out.println(permute.size());
    	if(permute.containsAll(selectedList)) {
    		permutationCount++;
    		if(!pause) {
    			display(permute);		//display a permutation
    		}
    	}
    	else {
    		while(it.hasNext()) {	    			
    			String s = it.next();
    			if(permute.contains(s)) {
    				continue;
    			}
    			permute.add(s);    			
    			getPermutation(select,permute);
    			permute.remove(s);
    		}
    	}
    }
    ...
    public void extendPermutation(List<String> permutationSoFar, Set<String> remaining){
    	...
    	if(remaining.isEmpty()) {
    		permutationCount++;
    		if(!pause) {
    			display(permutationSoFar);		//display a permutation
    		}
    		
    	}
    	else {    		
    		for(int i = 0; i < copy.size(); i++) {   			
    			remaining.remove(copy.get(i));		//remove the item from remaining items
    			permutationSoFar.add(copy.get(i));	//add it to the permutation so far
    			extendPermutation(permutationSoFar, remaining);	//do the recursive call
    			remaining.add(permutationSoFar.remove(permutationSoFar.size()-1));//remove the item from the end of the permutation and 
    																			  //put it back into the remaining items.
    		}
    	}
